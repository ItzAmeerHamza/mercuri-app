...
import { ActivityItemListTable } from "../../../components/dashboard/activity_item/activity-item-list-table";
...
const applyPagination = (activities, page, rowsPerPage) =>
...
const CustomerList = () => {
  ...
  const getAllActivityItems = async () => {
    let response =[]
    await axiosClient
      .get("Activity/getAllActivityItems", {})
      .then((res) => {
        response = res.data;
      })
      .catch((err) => {
...
// getCustomers
  const getActivities = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getAllActivityItems();
      if (isMounted()) {
        setActivities(data);
      }
  ...
  const sortedActivities = applySort(filteredActivities, sort);
  ...
  return (
    <>
      <Head>
        <title>Dashboard: Activity List | Material Kit Pro</title>
      </Head>
      <Box
        component="main"
        sx={{
